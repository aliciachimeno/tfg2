...
\noindent  The work revolves around the following theorem and its proof. 
	...
	\noindent Consider that $\sigma$ is not an algebraic polynomial and we aim to show that $\Sigma_n$ is dense in $\mathcal{C}(\mathbb{R}^n)$. In order to show that the set $\Sigma_n $ is dense in $\mathcal{C}(\mathbb{R}^n)$, we need to following Lemmas. \\ \\
	...
	\end{lema}
	\begin{proof}
		We first prove the claim in the case of $\varphi \in \mathcal{C}^\infty_0[a,b]$, for some $a<b$. \\ \\ By (\ref{prop:frech}) we have that $(\mathcal{C}^\infty_0[a,b],\rho)$ is a complete metric space.  \\ \\  Consider the following set, \\ $$V_k= \{\varphi \in \mathcal{C}_0^\infty[a,b] \, | \,  deg(\sigma \ast \varphi ) \leq k\}$$ \\ We want to show that $\mathcal{C}_0^\infty[a,b] \subseteq V_k.$ This set fulfills the following properties, $V_k \subset V_{k+1}$, $V_k $ is a closed subspace and $\cup_{k=0}^\infty V_k = \mathcal{C}_0^\infty[a,b]$. As $\mathcal{C}_0^\infty[a,b]$ is a complete metric space, by Baire's Category Theorem (\ref{baire}), this set is of category $II$, i.e. $\mathcal{C}_0^\infty[a,b]$  cannot be written as a countable union of nowhere-dense sets. We have $\cup_{k=0}^\infty V_k = \mathcal{C}_0^\infty[a,b]$. \textcolor{red}{aqui no entenc }Therefore, some $V_m$ contains a nonempty open set.  $V_m$ is a vector space thus $V_m=\mathcal{C}_0^\infty[a,b]$. \\ \\ 
