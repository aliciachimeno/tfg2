...
%\node [roundnode]  (A) {$\Sigma_n$ dens a $\mathcal{C}(\mathbb{R}^n)$};
%\node [roundnode]  (B) [right=of A] {$\sigma$ no és un polinomi};
%\node [roundnode]  (C) [below right=of B] {$\sigma  \ast \varphi $ no polinomi};
%\node [roundnode]  (D) [below left=of C] {$\sigma  \ast \varphi \in \overline{\Sigma_1}$};
%\node [roundnode]  (E) [left=of D] {$\sigma  \ast \varphi \in \mathcal{C}^\infty$};
%\node [roundnode]  (F) [above left=of E] {$\Sigma_1$ dens a $\mathcal{C}(\mathbb{R})$};
%	\path[<->] (A) edge (B);
%	\path[->] (B) edge (C);
%	\path[->] (C) edge (D);
%	\path[->] (D) edge (E);
